...
$("#toEditor").onclick=()=>{applyAxes();layoutInitial();renderMap();$("#summaryMember").textContent=state.member||"";$("#summaryMap").textContent=currentAxes().name;$("#allSizeRange").value=state.photos[0]?.size||58;show("screen-editor");};
...
$("#allSizeRange").oninput=e=>{
  const size=+e.target.value;
  state.photos.forEach(p=>p.size=size);
  $(".map-photo .pic").forEach(el=>{el.style.width=size+"px";el.style.height=size+"px";});
  const selected=state.photos.find(x=>x.id===state.selectedId);
  if(selected) $("#sizeRange").value=selected.size;
};
